...
- `bun install` — install dependencies (runs `electron-builder install-app-deps` postinstall).
- `bun dev` — start Vite dev servers + Electron for live reload during UI/IPC work.
- `bun start` — preview a built bundle in Electron using `electron-vite preview`.
- `bun build` — type-check both targets then bundle for production. Platform-specific: `bun build:mac`, `bun build:win`, `bun build:linux`; add `:unpack` to produce unpacked dirs.
- Quality: `bun lint` for ESLint, `bun format` for Prettier, `bun typecheck` (or `typecheck:node` / `typecheck:web`) for TS safety.

## Coding Style & Naming Conventions
- Use TypeScript everywhere; prefer typed IPC contracts in preload and avoid `any`.
- Prettier controls formatting (2-space indent, single quotes/semicolons per config); run `bun format` before committing.
...
## Testing Guidelines
- No automated test suite is configured yet; rely on `bun typecheck` and `bun dev` for manual validation.
