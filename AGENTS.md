...
- `npm run dev` — start Vite dev servers + Electron for live reload during UI/IPC work.
- `npm run start` — preview a built bundle in Electron using `electron-vite preview`.
- `npm run build` — type-check both targets then bundle for production. Platform-specific: `npm run build:mac`, `npm run build:win`, `npm run build:linux`; add `:unpack` to produce unpacked dirs.
- Quality: `npm run lint` for ESLint, `npm run format` for Prettier, `npm run typecheck` (or `typecheck:node` / `typecheck:web`) for TS safety.

## Coding Style & Naming Conventions
- Use TypeScript everywhere; prefer typed IPC contracts in preload and avoid `any`.
- Prettier controls formatting (2-space indent, single quotes/semicolons per config); run `npm run format` before committing.
...
## Testing Guidelines
- No automated test suite is configured yet; rely on `npm run typecheck` and `npm run dev` for manual validation.
